...
<div class="container mx-auto mt-10 px-6 ml-64">
    <div class="bg-white shadow-lg rounded-2xl p-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-semibold">📦 Barang Masuk</h2>
            <button onclick="toggleModal()" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">+ Tambah Barang Masuk</button>
        </div>

        <!-- Form Search -->
        <form method="GET" action="" class="mb-4">
            <input type="text" name="search" placeholder="Cari barang atau tanggal..." value="<?= htmlspecialchars($_GET['search'] ?? '') ?>" class="border p-2 rounded w-80">
                ...
                <tbody>
                    <?php if (!empty($rows)): $no=1; foreach ($rows as $r): ?>
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="border p-2"><?= htmlspecialchars($r['stok']) ?></td>
                            <td class="border p-2"><?= htmlspecialchars($r['nama_petugas']) ?></td>
                            <td class="border p-2"><?= htmlspecialchars($r['keterangan']) ?></td>
                        </tr>
                    <?php endforeach; else: ?>
                        <tr><td colspan="8" class="text-center p-4">Tidak ada data ditemukan.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
<!-- Modal Tambah Barang -->
<div id="modalTambah" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
